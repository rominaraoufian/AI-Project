...
def startturn(gridmap, height, width, turn, maxturn, timelimit):
    ...
    if len(diamond) == 0:
        ...
        ##check the codition wall//(heigh*width)<5 for other numbers
        depth = (floor(log((10**4) * timelimit, max(sizedh, 2)))-1) if ((walls//(height*width)*100) < 5) else floor(log((10**4) * timelimit, max(sizedh, 2)))
