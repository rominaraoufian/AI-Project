...
def trapornot(gridmap, height, width, next_move_agent, next_move_enemy, maxvalue, score_agent, score_enemy, start_agent, start_enemy, leastscore, diccolornumber_agent, diccolornumber_enemy, agent_trap, enemy_trap, character, character_enemy):
    ...
    #     (gridmap[next_move_enemy[0]][next_move_enemy[1]] != 'T') or (gridmap[next_move_enemy[0]][next_move_enemy[1]] != 'T' + character_enemy) ) and (not flagdiamond):
    #
    #         for item in dicfordiamond:
    #             if (item[2] == 10) and (dicfordiamond['y'] < 15) and (dicforall[(item[0],item[1])] != inf):
    #                 flagdiamond = True
    #             elif (item[2] == 25) and (dicfordiamond['g'] < 8) and (dicfordiamond[(item[0], item[1])][1] >= 15):
    #                 flagdiamond = True
    #             elif (item[2] == 35) and (dicfordiamond['r'] < 5) and (dicfordiamond[(item[0], item[1])][1] >= 50):
    #                 flagdiamond = True
    #             elif (item[2] == 75) and (dicfordiamond['b'] < 4) and (dicfordiamond[(item[0], item[1])][1] >= 140):
    #                 flagdiamond = True
